...
pub fn cx_bn_alloc_init(nbytes: u32, value: &[u8]) -> Result<CxBn, SyscallError> {
	let mut x = CxBn::new();
    let err = unsafe { bindings::cx_bn_alloc_init(&mut x.x, nbytes, value.as_ptr(), value.len() as u32) };
	if err != 0 {
		Err(err.into())
	} else {
		Ok(x)
...
pub fn cx_ecpoint_init(p: &mut bindings::cx_ecpoint_t, x: &[u8], y: &[u8]) -> Result<(), SyscallError> {
    let err = unsafe { bindings::cx_ecpoint_init(p as *mut bindings::cx_ecpoint_t, x.as_ptr(), x.len() as u32, y.as_ptr(), y.len() as u32) };
	if err != 0 {
		Err(err.into())
	} else {
		Ok(())
...
pub fn cx_ecpoint_add(r: &mut bindings::cx_ecpoint_t, p: &bindings::cx_ecpoint_t, q: &bindings::cx_ecpoint_t) -> Result<(), SyscallError> {
    let err = unsafe { bindings::cx_ecpoint_add(r as *mut bindings::cx_ecpoint_t, p as *const bindings::cx_ecpoint_t, q as *const bindings::cx_ecpoint_t) };
	if err != 0 {
		Err(err.into())
	} else {
		Ok(())
...
pub fn cx_ecpoint_export(p: &bindings::cx_ecpoint_t, x: &mut [u8], y: &mut [u8]) -> Result<(), SyscallError> {
    let err = unsafe { bindings::cx_ecpoint_export(p, x.as_mut_ptr(), x.len() as u32, y.as_mut_ptr(), y.len() as u32) };
	if err != 0 {
		Err(err.into())
	} else {
		Ok(())
